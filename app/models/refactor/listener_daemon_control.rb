...
# Value of ARGV[4] => listener key
    ...
    logger = Logger.new(File.join("#{rails_root}", "log", "listener_daemon_control.log"))
    ...
    options = {
        ...
        :dir            => File.join("#{rails_root}", "tmp", "pids"),    #here is where we keep the pids
        :multiple       => false,                                        #this will allow multiple daemons to run as distinct entities
        :ontop          => false,                                        #
        :mode           => :load,
        :backtrace      => false,
        :monitor        => false,
    ...
    target = File.join("#{rails_root}", "app", "models", "listener_daemon.rb")
